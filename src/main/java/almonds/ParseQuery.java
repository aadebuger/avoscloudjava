...
public class ParseQuery
{
	...
	public String mWheresEqualTo = null;
...
//			httpget.addHeader("X-Parse-REST-API-Key", Parse.getRestAPIKey());
			...
			httpget.addHeader("X-AVOSCloud-Application-Id", Parse.getApplicationId());
			httpget.addHeader("X-AVOSCloud-Application-Key", Parse.getRestAPIKey());
			...
			try
			{
				...
				for (int i = 0; i < results.length(); i++)
				{
					...
					{
						System.out.println("jsonObject type"+ jsonObject.get(name).getClass().getName());
						if (jsonObject.get(name).getClass().getName().equals("org.json.JSONArray"))
						{
							parseObject.put(name, jsonObject.get(name).toString());
						}
							else
	...
	public List<ParseObject> find(String oid) throws ParseException
	{
		try
		{
			HttpClient httpclient = new DefaultHttpClient();
			HttpGet httpget = new HttpGet(Parse.getParseAPIUrlClasses() + mClassName+ "/"+oid);
//			httpget.addHeader("X-Parse-Application-Id", Parse.getApplicationId());
//			httpget.addHeader("X-Parse-REST-API-Key", Parse.getRestAPIKey());
	...
	{
		return  hasWheresConstraints()|| hasWhereConstraints() || hasOrderConstraints() || hasLimitConstraints()
				|| hasSkipConstraints();
	}
	...
	private boolean hasWheresConstraints()
	{
		return mWheresEqualTo != null;
	}
	...
	private String getURLConstraints()
	{
		...
		try
		{
			...
			if (hasConstraints())
			{
				...
				if (hasWheresConstraints())
				{
					...
					url += "where=" + URLEncoder.encode(mWheresEqualTo, "UTF-8");
				}
		...
		System.out.println("parseObject url="+url);
		return url;
	}
